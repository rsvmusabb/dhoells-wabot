...
// Checks: full JID, LID, bare number, with device suffix (:xx)
// ============================================================
function isOwner(jid) {
    if (!jid) return false;
    const parts = jid.split('@');
    const beforeAt = parts[0].split(':')[0];
    const ownerClean = BOT_CONFIG.OWNER_NUMBER.split('@')[0];
    const ownerLid = (BOT_CONFIG.OWNER_LID || '').split('@')[0];
    const result = beforeAt === ownerClean || beforeAt === ownerLid
        || jid === BOT_CONFIG.OWNER_NUMBER
        || jid === BOT_CONFIG.OWNER_LID;
    if (!result && (beforeAt === '143362233520260' || beforeAt === '6285975216127')) {
        console.log(`[IS_OWNER_DEBUG] jid=${jid} beforeAt=${beforeAt} ownerClean=${ownerClean} ownerLid=${ownerLid} result=${result}`);
    }
    return result;
}
