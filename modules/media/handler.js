...
async function handlePlay(sock, sender, msg, participant, textMessage) {
    if (!textMessage) { await sock.sendMessage(sender, { text: '📌 Masukkan judul lagu atau URL YouTube!\nContoh: *.play akad payung teduh*' }, { quoted: msg }); return; }
    ...
    try {
        let v = null;
        const ytRegex = /(?:youtu\.be\/|youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=))([^"&?\/\s]{11})/i;
        const match = textMessage.match(ytRegex);
        
        if (match && match[1]) {
            v = await yts({ videoId: match[1] });
        } else {
            const sr = await yts(textMessage);
            v = sr.videos[0];
        }

        if (!v || (!v.url && !v.videoId)) { 
            await sock.sendMessage(sender, { text: '❌ Lagu tidak ditemukan.' }, { quoted: msg }); 
            return; 
        }
        
        const videoUrl = v.url || `https://www.youtube.com/watch?v=${v.videoId}`;
        await sock.sendMessage(sender, { text: `🎧 *Mendownload...*\n\n*Judul:* ${v.title || 'Lagu'}\n*Durasi:* ${v.timestamp || v.duration?.timestamp || 'Unknown'}` }, { quoted: msg });
        of = await downloadAudio(videoUrl); 
        const ab = fs.readFileSync(of);
